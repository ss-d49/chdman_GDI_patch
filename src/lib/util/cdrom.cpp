...
constexpr std::array<uint32_t, cdrom_file::EDC_CRCTABLE_SIZE> cdrom_file::edc_crctable_gen(void)
{
 ...
 */
void cdrom_file::edc_generate(uint8_t *sector)
{
	const auto EDC_crctable = edc_crctable_gen();
	uint32_t result = 0;
...
//-------------------------------------------------
void cdrom_file::gdrom_convert_toc(cdrom_file::toc *toc)
{
	...
	{
		toc->tracks[trknum].multicuearea = ((trknum < 2) ? cdrom_file::SINGLE_DENSITY : cdrom_file::HIGH_DENSITY);
	...
	{
		if (toc->tracks[trknum].pgtype == cdrom_file::CD_TRACK_MODE1_RAW)
		{
			// un-pad DATA track
		...
		if (toc->tracks[trknum].pgtype == cdrom_file::CD_TRACK_AUDIO && toc->tracks[trknum+1].pgtype == cdrom_file::CD_TRACK_AUDIO)
		{
			// un-shift consecutive AUDIO tracks
			toc->tracks[trknum].splitframes = toc->tracks[trknum].pregap;
		}

		if (toc->tracks[trknum].pgtype == cdrom_file::CD_TRACK_AUDIO && toc->tracks[trknum+1].pgtype == cdrom_file::CD_TRACK_MODE1_RAW)
		{
	...
	switch (cdrom_file::gdrom_identify_pattern(toc))
	{
		case cdrom_file::TYPE_I:
			// do nothing
			break;

		case cdrom_file::TYPE_II:
			// un-shrink final AUDIO track
			toc->tracks[toc->numtrks-1].frames += toc->tracks[toc->numtrks-1].pregap;
			break;

		case cdrom_file::TYPE_III:
		...
		case cdrom_file::TYPE_III_SPLIT:
