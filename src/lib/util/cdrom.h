...
// EDC utilities
static void edc_generate(uint8_t *sector);
...
static void gdrom_convert_toc(toc *toc);
static enum gdi_pattern gdrom_identify_pattern(const toc *toc);
private:



	/** @brief  EDC offsets */
	static constexpr int EDC_MODE1_OFFSET = 16+2048;
	/** @brief  EDC CRC table size */
	static constexpr int EDC_CRCTABLE_SIZE = 256;
	/** @brief  EDC polynomial as per ECMA 130 */
	static constexpr uint32_t EDC_POLYNOMIAL = 0x8001801bL;
